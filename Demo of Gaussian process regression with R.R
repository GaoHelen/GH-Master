...
names(values) <- c("x","Function 1","Function 2","Function 3")
values <- values %>% 
  gather(key="Sampled Functions",value=y, `Function 1`:`Function 3`)

# Plot the result
ggplot(values,aes(x=x,y=y)) +
  geom_line(aes(group=`Sampled Functions`, linetype=`Sampled Functions`, color=`Sampled Functions`)) +
  theme_bw() +
...
ggplot(values,aes(x=x,y=y)) +
  ...
  geom_line(data=data.frame(x.star=x.star,f.star.bar=f.star.bar),
            aes(x=x.star,y=f.star.bar),colour="red", size=1) + 
